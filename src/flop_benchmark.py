...
def get_model_infos(model, one_input):
  ...
  with torch.no_grad():
    _____ = model('search', input_ids, segment_ids, input_mask, is_student=True)
    # _____ = model(input_ids)
...
def compute_average_flops_cost(model):
  ...
  for module in model.modules():
    if isinstance(module, torch.nn.Linear) or hasattr(module, 'calculate_flop_self'):
...
def fc_flops_counter_hook(fc_module, inputs, output):
  batch_size = inputs[0].size(-2)
  xin, xout = fc_module.in_features, fc_module.out_features
  assert xin == inputs[0].size(-1) and xout == output.size(-1), 'IO=({:}, {:})'.format(xin, xout) + '\tin,out=({}, {})'.format(inputs[0].size(), output.size())
