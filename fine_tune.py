...
def main():
    ...
    parser.add_argument("--test_output_dir",
                        ...
                        type=str,
                        required=True,
                        help="The output directory where the model testing predictions will be written.")
    ...
    model = BertForSequenceClassification.from_pretrained(args.bert_model,
              cache_dir=args.cache_dir +'/distributed_{}'.format(args.local_rank),
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                model_out = model(input_ids, segment_ids, input_mask, label_ids)
                loss = model_out[0]
                if n_gpu > 1:
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_data, all_label_ids = get_tensor_data(output_mode, eval_features)
        ...
        if task_name != "sts-b":
            ...
            preds = preds[0]
            preds = np.squeeze(preds)
            eval_metric = pearson_and_spearman(preds, eval_labels.numpy())['spearmanr']
    ...
    if args.do_test:
        ...
        test_features = convert_examples_to_features(
            test_examples, label_list, args.max_seq_length, tokenizer, output_mode, test=True)
        ...
        model.eval()
        do_test(model, args.task_name, test_dataloader, device, output_mode, num_labels, args.test_output_dir, processor.get_labels())
# Calls Main function
